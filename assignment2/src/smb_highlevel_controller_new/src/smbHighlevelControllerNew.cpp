...
smbHighlevelControllerNew::smbHighlevelControllerNew(ros::NodeHandle& nodeHandle) :
  nodeHandle_(nodeHandle) {
  	// to obtain the params by calling getParam()
  	nodeHandle.getParam("/smb_highlevel_controller_new/scan_subscriber_topic", scan_subscriber_topic);
  	nodeHandle.getParam("/smb_highlevel_controller_new/scan_subscriber_queue_size", scan_subscriber_queue_size);
	...
	scan_subscriber_ = nodeHandle_.subscribe(scan_subscriber_topic, scan_subscriber_queue_size, &smbHighlevelControllerNew::scanCallback, this);
		ROS_INFO("Successfully launch node");
    }

// exercise 2-6: create a callback method to record the minimum distance of scan range
void smbHighlevelControllerNew::scanCallback(const sensor_msgs::LaserScan &scan_msg) {
...
smbHighlevelControllerNew::~smbHighlevelControllerNew() {}
